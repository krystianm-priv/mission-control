...
test(
	...
	const mission = m
		...
		.start({ input: { parse: parseStart }, run: async ({ ctx }) => ({ id: ctx.events.start.input.id }) })
		.needTo("approve", { parse: parseApproval })
		.step("finish", async ({ ctx }) => ({ approvedBy: ctx.events.approve.input.approvedBy }))
		.end();
	...
	try {
		...
		const retrieved = await commander2.getMission<typeof mission>("durable-id-1");
		assert.ok(retrieved);
		if (!retrieved) {
...
test(
	...
	const mission = m
		...
		.sleep("pause", 25)
		.needTo("approve", { parse: parseApproval })
		.step("finish", async ({ ctx }) => ({ id: ctx.events.start.output.id, approvedBy: ctx.events.approve.input.approvedBy }))
		.end();
	...
	try {
		...
		const recovered = await commander2.getMission<typeof mission>("durable-id-3");
		assert.ok(recovered);
		if (!recovered) {
...
test(
	...
	try {
		...
		const commander2 = new SQLiteCommander({ databasePath: temp.path });
		const loaded = await commander2.getMission<typeof mission>("durable-id-4");
