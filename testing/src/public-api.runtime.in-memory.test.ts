...
test(
	...
	const created = commander.createMission(mission, { missionId: "m-retrieve-1" });
	await created.start({ id: "abc" });

	const retrieved = await commander.getMission<typeof mission>("m-retrieve-1");
	assert.ok(retrieved);
	if (!retrieved) {
	...
	assert.deepEqual(retrieved.inspect().snapshot.ctx.events["finish"]?.output, {
		id: "abc",
		approvedBy: "ops",
	});
},
...
test(
	...
	const first = commander.createMission(mission, { missionId: "duplicate-1" });
