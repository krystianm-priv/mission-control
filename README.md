...
- source code uses erasable TypeScript syntax only; npm packages publish built ESM output generated by `tsc`
...
3. Smoke-test the built tarballs in a clean consumer project on Node.js 24+.
4. Ensure docs and changelog are updated for API/semantics changes.
5. Keep internal `@mission-control/*` package dependencies on exact matching versions for published packages.
6. Human operators perform dependency updates, generated-file actions, lockfile modifications, and publish commands.
